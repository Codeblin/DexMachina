"""DroidForge interactive console — a pentest REPL for a connected device.
...
from droidforge.bypass import BypassError, list_android_apps, run_bypass
...
from droidforge.runtime import RunError, build_run_env, run_invocation
from droidforge.versions import get_active_frida_version

console = Console()

BANNER = r"""
[bold #00ff41]⚔  DroidForge Console[/]  [dim]— interactive Android pentest shell[/]
"""
...
class DroidForgeConsole(cmd.Cmd):
    ...
    def _update_prompt(self) -> None:
        ...
        self.prompt = f"{_GREEN}droidforge{_RESET} {_DIM}[{dev} | {tgt}]{_RESET}> "
    ...
    def do_status(self, arg: str) -> None:
        ...
        lines = [
            ...
            f"Frida runtime : {active or '[yellow]none — run: droidforge use latest[/]'}",
    ...
    def do_ready(self, arg: str) -> None:
        ...
        if not get_active_frida_version(self.config):
            console.print(
                ...
                "[cyan]droidforge use latest[/] (outside the console), then re-run [cyan]ready[/]."
            )
            return
        try:
    ...
    def do_run(self, arg: str) -> None:
        """run <tool> [args...] — run any DroidForge tool (frida, jadx, apktool, …)."""
        args = self._split(arg)
        if not args:
    ...
    def do_exit(self, arg: str) -> bool:
        """exit — leave the DroidForge console."""
        console.print("[dim]Leaving DroidForge console.[/]")
        return True
...
def run_console(config: dict, serial: str | None = None) -> int:
    """Start the interactive DroidForge console."""
    if not (sys.stdin and sys.stdin.isatty()):
        console.print(
            "[yellow]The console needs an interactive terminal.[/] "
            "Run [cyan]droidforge console[/] directly in your terminal."
        )
        return 1
    DroidForgeConsole(config, serial=serial).cmdloop()
    return 0
