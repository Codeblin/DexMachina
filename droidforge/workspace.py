"""Repo-local workspace setup — `droidforge init` and .gitignore management."""
...
from droidforge.config import DEFAULT_CONFIG_NAME
from droidforge.profiles import DEFAULT_PROFILE

# Everything DroidForge downloads lives under this dir; it is gitignored.
WORKSPACE_DIRNAME = ".droidforge"
TOOLS_SUBDIR = "tools"
CACHE_SUBDIR = "cache"

GITIGNORE_BLOCK_HEADER = "# DroidForge - downloaded tools & caches (managed)"
...
def _project_config_body(profile: str) -> str:
    return (
        "# DroidForge project environment\n"
        "# Tools are downloaded into .droidforge/tools and added to PATH via\n"
        "#   droidforge env   (print)   or   droidforge shell   (subshell)\n"
        "\n"
        "[settings]\n"
        'adb_path = "adb"\n'
        'java_path = "java"\n'
...
def update_gitignore(repo_dir: Path) -> tuple[Path, list[str]]:
    """Ensure DroidForge entries exist in repo .gitignore. Returns (path, added)."""
...
def init_workspace(
    ...
    *,
    ...
    """Create a repo-local DroidForge workspace (config, dirs, gitignore)."""
