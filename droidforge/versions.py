...
def venvs_root() -> Path:
    return Path.home() / ".droidforge" / "venvs"
...
def env_shell_hint(config: dict) -> str:
    ...
    ver = get_active_frida_version(config)
    if not ver:
        return "No active frida version. Run: droidforge use <version>  (e.g. droidforge use 17.11.0)"
...
def print_versions_report(config: dict) -> None:
    ...
    from droidforge.installer import get_latest_version, get_tool_version
    ...
    console.print(Panel(
        ...
        "Think [bold]nvm[/]: [cyan]droidforge use 17.11.0[/] creates an isolated venv and "
        "sets it active. Switch anytime with [cyan]droidforge use 16.2.0[/].",
    ...
    console.print("  [cyan]droidforge use 17.11.0[/]     switch/create venv (like nvm use)")
    console.print("  [cyan]droidforge use latest[/]     use latest frida release")
    console.print("  [cyan]droidforge pin frida X[/]    lock config without reinstall")
    console.print("  [cyan]droidforge env[/]            print PATH to activate active venv")
    console.print("  [cyan]droidforge sync frida[/]     align global pip install to target")
...
def print_sync_error(err: PinGroupSyncError) -> None:
    ...
    lines.extend([
        "",
        "[bold]What to do:[/]",
        "  1. [cyan]droidforge use {ver}[/]  — nvm-style venv (recommended)".format(
            ver=err.target_frida
        ),
        "  2. [cyan]droidforge sync frida[/]  — fix global pip install",
        "  3. [cyan]droidforge versions frida[/]  — inspect group + available releases",
        "  4. [cyan]droidforge pin frida {ver}[/]  — lock version, then use/sync".format(
            ver=err.target_frida
        ),
    ])
