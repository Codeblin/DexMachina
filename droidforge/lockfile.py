...
from droidforge.config import config_root, get_setting
from droidforge.registry import TOOLS, get_tool

LOCK_NAME = "droidforge.lock.toml"
...
def build_lock(config: dict) -> dict[str, Any]:
    ...
    for name, tool in TOOLS.items():
        ...
        if version:
            tools[name] = {"version": version, "method": tool.install_method}

    lock: dict[str, Any] = {
        "meta": {
            "droidforge_version": __version__,
...
def restore_from_lock(config: dict, lock: dict[str, Any]) -> tuple[list[str], list[tuple[str, str]]]:
    ...
    """
    from droidforge.installer import InstallError, install_tool
    from droidforge.versions import use_frida_version
    ...
    for name, info in locked_tools.items():
        ...
        try:
            ...
            install_tool(name, config, version=pin)
