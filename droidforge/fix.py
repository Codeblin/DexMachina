...
from droidforge.config import get_pinned_version, is_ignored
from droidforge.device import push_frida_server
from droidforge.doctor import CheckResult, run_doctor
...
from droidforge.registry import FRIDA_PIN_GROUP, get_tool, list_tools
from droidforge.utils import which
...
RISK_LEVELS: dict[str, dict[str, str]] = {
    ...
    "high": {
        ...
        "detail": (
            "OS-level installs, hardware setup, or commercial tools DroidForge cannot "
        ...
        for tool_name in CORE_ENV_TOOLS:
            ...
            add(
                FixAction(
                    ...
                    description="Part of the DroidForge pentest environment baseline",
                    risk="low",
...
def print_fix_plan(plan: list[FixAction], *, show_legend: bool = True) -> None:
    ...
    table = Table(
        title="[bold cyan]⚙ DroidForge Fix Plan[/]",
        show_header=True,
