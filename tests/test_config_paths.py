...
def test_relative_install_dir_resolves_against_config_root(tmp_path):
    cfg = _cfg(tmp_path, ".pindroid/tools")
    assert expand_path(cfg, "install_dir") == (tmp_path / ".pindroid" / "tools").resolve()
...
def test_home_path_expands(tmp_path):
    cfg = _cfg(tmp_path, "~/.pindroid/tools")
    assert expand_path(cfg, "install_dir") == (Path.home() / ".pindroid" / "tools").resolve()


def test_config_root_falls_back_to_home_without_meta():
    cfg = {"settings": {}}
    assert config_root(cfg) == Path.home() / ".pindroid"


def test_is_project_config(tmp_path):
    project = _cfg(tmp_path, ".pindroid/tools")
