...
from droidforge.installer import check_pin_group_conflict, force_version_match
...
def test_pin_conflict_empty_when_no_installed():
    ...
    with patch("droidforge.installer.get_tool_version", return_value=None):
...
def test_pin_conflict_detects_mismatch():
    ...
    with patch("droidforge.installer.get_tool_version", side_effect=fake_version):
        warnings = check_pin_group_conflict("frida", "16.2.0", cfg)
